...
module.exports = (gulp, g, config, paths, typingsStream, debug, _, sassPipe, state, lib) => {
    ...
    function createFuseBox() {
        return new fsbx.FuseBox({
            homeDir: config.projectRoot + '/src',
            sourceMap: {
                bundleReference: "sourcemaps.js.map",
                outFile: "./build/js/sourcemaps.js.map",
            },
            cache: true,
    ...
    const fuseBox = _.once(createFuseBox);

    gulp.task("scripts", function scripts() {
        const app = fuseBox().bundle(">app/main.ts")
            .then(function(result) {
                return result && result.content;
            });
        return streamFromPromise(app)
